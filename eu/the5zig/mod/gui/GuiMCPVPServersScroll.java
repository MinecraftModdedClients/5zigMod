...
public abstract class GuiMCPVPServersScroll
  extends GuiMCPVPServers
{
  ...
  protected void refreshServers()
  {
    ...
    this.joinSelectedServerButton.setEnabled(false);
  ...
  public void run()
  {
    ...
    this.joinSelectedServerButton.setEnabled(this.serverSlot.getSelectedRow() != null);
  }
  
  public void initGui()
  {
    initButtons();
    this.serverSlot = The5zigMod.getVars().createGuiList(new Clickable()
    {
      public void onSelect(int id, MCPVPServer row, boolean doubleClick)
      {
        if (row == null)
        {
          GuiMCPVPServersScroll.this.joinSelectedServerButton.setEnabled(false);
          return;
        }
        GuiMCPVPServersScroll.this.joinSelectedServerButton.setEnabled(true);
        if (doubleClick) {
          GuiMCPVPServersScroll.this.joinServer(id);
        }
      }
    }, getWidth(), getHeight(), 32, getHeight() - 64, 0, getWidth(), this.servers);
    this.serverSlot.setScrollX(getWidth() / 2 + 124);
    this.serverSlot.setRowWidth(220);
    addGuiList(this.serverSlot);
    refreshServers();
  }
  ...
  protected void initButtons()
  {
    ...
    addButton(The5zigMod.getVars().createButton(0, getWidth() / 2 + 82, getHeight() - 40, 72, 20, The5zigMod.getVars().translate("gui.cancel", new Object[0])));
    this.joinSelectedServerButton.setEnabled(false);
  }
  ...
  protected void actionPerformed(IButton button)
  {
    if (button.getId() == 1) {
      ...
      The5zigMod.getVars().displayScreen(new ServerListMCPvP(this.lastScreen));
